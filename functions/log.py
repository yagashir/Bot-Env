...
class Log:
    ...
    def records(self, entry_price, exit_price, trade_cost, data, position_info, backtest_log, close_type=None):
        ...
        if position_info["side"] == "BUY":
            ...
            backtest_log["profit"].append(buy_profit)
            backtest_log["return"].append(round(buy_profit / entry_price * 100, 4))
            backtest_log["funds"] += buy_profit

        if position_info["side"] == "SELL":
            backtest_log["side"].append("SELL")
            backtest_log["profit"].append(sell_profit)
            backtest_log["return"].append(round(sell_profit / entry_price * 100, 4))
            backtest_log["funds"] += sell_profit
